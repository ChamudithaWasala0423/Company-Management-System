		...
		<form method="post" id="create_invoice">
			...
			<div class="row">
				...
				<div class="col-xs-8 text-right">
					...
					$sql_one = "SELECT * FROM job_registartion ORDER BY job_number ASC";
					$result_one = mysqli_query($con, $sql_one);
					
					echo'
					<div class="input-group col-xs-4 float-right">
						<span class="input-group-addon">#JOBN</span>
						<select name="job_id" id="job_id" class="form-control required">';
						while($row_one =  $result_one->fetch_assoc()){
							echo'<option value="'.$row_one['job_number'].'">'.$row_one['job_number'].'-'.$row_one['customer_name'].'-'.$row_one['location'].'</option>';
						}
						echo'
						</select>
					</div>';
